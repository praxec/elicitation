//! The event-log persistence seam (spec §7 / §8).
//!
//! [`StateStore`] is the only I/O boundary the kernel knows about: append one
//! event, replay a session's events, list sessions, check existence. The
//! filesystem impl writes one JSON event per line to
...
pub trait StateStore: Send + Sync {
    ...
    /// [`EngineError::SessionNotFound`] if the session has never been opened.
...
/// with the `.jsonl` layout. The kernel keys every file by `session_id`, so
/// this is the durability boundary's safety check (spec §8).
pub fn validate_session_id(session_id: &str) -> Result<()> {
    if session_id.is_empty() {
        return Err(EngineError::BadSessionId("empty session id".into()));
    }
    ...
    if has_illegal_char || session_id.contains("..") || session_id.starts_with('.') {
        return Err(EngineError::BadSessionId(format!(
...
/// Filesystem-backed [`StateStore`] (spec §8). One `.jsonl` file per session
/// under `state_dir`.
#[derive(Debug, Clone)]
...
impl FilesystemStore {
    ...
    pub fn new(state_dir: impl Into<PathBuf>) -> Result<Self> {
        ...
        fs::create_dir_all(&state_dir)
            .map_err(|e| EngineError::StoreError(format!("create_dir_all {state_dir:?}: {e}")))?;
        Ok(Self { state_dir })
    }
...
impl StateStore for FilesystemStore {
    fn append(&self, session_id: &str, event: &Event) -> Result<()> {
        ...
        let line = serde_json::to_string(event)
            .map_err(|e| EngineError::StoreError(format!("serialize event: {e}")))?;
        let mut file = OpenOptions::new()
            .create(true)
            .append(true)
            .open(&path)
            .map_err(|e| EngineError::StoreError(format!("open {path:?}: {e}")))?;
        writeln!(file, "{line}")
            .map_err(|e| EngineError::StoreError(format!("write {path:?}: {e}")))?;
        file.flush()
            .map_err(|e| EngineError::StoreError(format!("flush {path:?}: {e}")))?;
        Ok(())
    }
    ...
    fn replay(&self, session_id: &str) -> Result<Vec<Event>> {
        ...
        for (idx, line) in reader.lines().enumerate() {
            let line = line.map_err(|e| EngineError::StoreError(format!("read line: {e}")))?;
            if line.trim().is_empty() {
                continue;
            }
            let event: Event = serde_json::from_str(&line).map_err(|e| {
                EngineError::StoreError(format!("parse {path:?} line {}: {e}", idx + 1))
            })?;
            events.push(event);
        }
        Ok(events)
...
mod tests {
    ...
    fn opened(session_id: &str) -> Event {
        Event::SessionOpened {
            ...
            coverage_schema: CoverageSchema::default(),
        }
    }
